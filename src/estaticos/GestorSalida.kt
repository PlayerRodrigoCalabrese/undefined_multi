...
object GestorSalida {
    ...
    fun ENVIAR_Af_NUEVA_COLA(
        out: PrintWriter?, posicion: Int, totalAbo: Int,
        totalNonAbo: Int, suscribirse: String, colaID: Int
    ) {
        val packet =
            "Af$posicion|$totalAbo|$totalNonAbo|$suscribirse|$colaID"
        enviar(out, packet)
        if (MainMultiservidor.MOSTRAR_ENVIOS) {
    ...
    fun ENVIAR_Ad_Ac_AH_AlK_AQ_INFO_CUENTA_Y_SERVER(
        out: PrintWriter?, apodo: String?,
        rango: Int, pregunta: String
    ) {
        val packet = StringBuilder()
        packet.append("Ad").append(apodo).append(0x00.toChar())
        packet.append("Ac" + "??" + 0x00.toChar())
        packet.append("AH").append(Mundo.packetParaAH()).append(0x00.toChar())
        packet.append("AlK").append(rango).append(0x00.toChar())
        packet.append("AQ").append(pregunta.replace(" ", "+"))
    ...
    fun ENVIAR_AXK_O_AYK_IP_SERVER(
        out: PrintWriter?, cuentaID: Int, ipServer: String?,
        puertoServer: Int
    ) {
        val packet = StringBuilder("A")
        if (MainMultiservidor.ENCRIPTAR_IP) {
            packet.append("XK").append(ipServer?.let { Encriptador.encriptarIP(it) })
                .append(Encriptador.encriptarPuerto(puertoServer)).append(cuentaID)
        } else {
            packet.append("YK").append(ipServer).append(":").append(puertoServer).append(";").append(cuentaID)
        }
        enviar(out, packet.toString())
        if (MainMultiservidor.MOSTRAR_ENVIOS) {
    ...
    fun ENVIAR_M1_MENSAJE_SERVER(
        out: PrintWriter?, id: String, msj: String,
        nombre: String
    ) {
